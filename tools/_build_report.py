...
# Read current file to keep the FPDF class, we'll replace the LLM/analysis part
with open(path, "r", encoding="utf-8") as f:
    content = f.read()

# New content: replace everything from the imports down
new_content = r'''
"""
...
def _llm_analyze_response(part, question_info, user_response, topic=""):
    ...
    system_prompt = (
        ...
        "Return your analysis in the EXACT structured format below, with each section clearly marked."
    )
    ...
    user_prompt = (
        ...
        + "---FLUENCY---\n"
        + "Score: [1-9]\n"
        + "Analysis: [2-3 sentences analyzing the response]\n"
        ...
        + "IMPORTANT: Be honest and accurate. Use ONLY the EXACT section headers shown above."
...
class CorrectionReport(FPDF):
    ...
    def __init__(self):
        ...
        _r = os.path.join(_fd, "segoeui.ttf")
        _rb = os.path.join(_fd, "segoeuib.ttf")
        _ri = os.path.join(_fd, "segoeuii.ttf")
...
print("correction_report.py rewritten with LLM support")
